...
blink_detector = BlinkDetector(serial_port="COM6", eog_channel=2)

#Ruta absoluta con imagenes y audios - Ajustar
...
while running:
    ...
    if not game_over:
        ...
        #Disparo (BLINK) con pequeño cooldown
        if bullet_cooldown > 0:
            bullet_cooldown -= 1
        blink = blink_detector.update()

        if blink and not keys[pygame.K_SPACE] and bullet_cooldown == 0:
           spawn_bullet()
           bullet_cooldown = 88
